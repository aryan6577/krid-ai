...
// assistant's last reply is clearly asking for a venue or date/time, so match
// proposal flow can be completed in taps.
...
export function getQuickReplies({ lastBotText = "", role, currentPlayer, venues = [], friendPlayers = [], funding = [], career = [], sponsorships = [] }) {
  ...
  const asksVenue = /(which|what).{0,15}(venue|turf|court)/.test(text) || (text.includes("venue") && text.includes("proposal"));
  const asksDateTime = /(what|which).{0,10}(date|time)|when would you like|what time/.test(text);

  if (asksVenue && topVenues.length) {
    return topVenues.map((v) => `Plan ${sport} near ${v.location} today at ${DEFAULT_TIME}`);
  }
  ...
  const chips = [];
  if (topVenues[0]) chips.push(`Plan ${sport} today at ${DEFAULT_TIME}`);
  if (acceptedFriends[0]) chips.push(`Find ${sport} players for this weekend`);
